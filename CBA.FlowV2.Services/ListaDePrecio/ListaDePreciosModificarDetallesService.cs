...
namespace CBA.FlowV2.Services.ListaDePrecio
{
    public class ListaDePreciosModificarDetallesService
    {
        ...
        public void AgregarPorListaPrecios(decimal lista_precio_modificar_id, decimal lista_precios_id, decimal porcentajeModificacion)
        {
            ...
            DataTable dtListaPrecioDet = ListasDePrecioDetalleService.GetListasDePrecioDetalleDataTable(clausulas, string.Empty); // (new ListasDePrecioDetalleService()).GetListasDePrecioDetalleInfoCompleta(clausulas, string.Empty);
            // si aqui no hay registros, es porque en el caso ya se agregaron todos los articulos existentes en la lista de precio
            if (dtListaPrecioDet.Rows.Count == 0)
                return;

            DataTable dtIDs = dtListaPrecioDet.DefaultView.ToTable("DistinctTable", true, ListasDePrecioDetalleService.ArticuloId_NombreCol);

            clausulas = ListasDePrecioDetalleService.ListaPrecioId_NombreCol + " = " + lista_precios_id;

            if (dtIDs.Rows.Count > 0)
                clausulas += " and " + ListasDePrecioDetalleService.ArticuloId_NombreCol + " in (" + dtIDs.Rows[0][ListaDePreciosModificarDetallesService.ArticuloId_NombreCol];
            for (int i = 1; i < dtIDs.Rows.Count; i++)
                clausulas += ", " + dtIDs.Rows[i][ListaDePreciosModificarDetallesService.ArticuloId_NombreCol];
            if (dtIDs.Rows.Count > 0)
                clausulas += ") ";

            clausulas += " and " + ListasDePrecioDetalleService.Estado_NombreCol + " = '" + Definiciones.Estado.Activo + "'";
            string clausulaAux = clausulas;
         //   clausulas += " and " + ListasDePrecioDetalleService.SucursalId_NombreCol + " is null ";

            dtListaPrecioDet = ListasDePrecioDetalleService.GetListasDePrecioDetalleDataTable(clausulas, ListasDePrecioDetalleService.SucursalId_NombreCol +" desc, "+ListasDePrecioDetalleService.Id_NombreCol);
           // if (dtListaPrecioDet.Rows.Count == 0)//volvemos a preguntar en caso de que no existan precios cuando sucursal es null, entonces cargamos el primero que se encuentra
             //   dtListaPrecioDet = ListasDePrecioDetalleService.GetListasDePrecioDetalleDataTable(clausulaAux, ListasDePrecioDetalleService.SucursalId_NombreCol);
            ...
            for (int i = 0; i < dtListaPrecioDet.Rows.Count; i++)
            {
                ...
                campos.Add(ListaDePreciosModificarDetallesService.PrecioNuevo_NombreCol, /*(decimal)0);*/dtListaPrecioDet.Rows[i][ListasDePrecioDetalleService.PrecioCalculado_NombreCol]);
                ...
                campos.Add(ListaDePreciosModificarDetallesService.FechaInicio_NombreCol, DateTime.Now);

                //Controla que no se ingrese mas de una vez el mismo articulo
                string clausula = ListaDePreciosModificarDetallesService.ListaPreciosModificarId_NombreCol + " = " + campos[ListaDePreciosModificarDetallesService.ListaPreciosModificarId_NombreCol] + " and " +
                ...
                if (this.GetListaDePreciosModificarDetallesDataTable(clausula, string.Empty).Rows.Count > 0)
                    continue;
        ...
        public DataTable GetListaDePreciosModificarDetalleDataTable2(string clausulas, string orden)
        {
            ...
            using (SessionService sesion = new SessionService())
            {
                ...
                sql.Append("a.monto_precio_padre, \n");
        ...
        public void Guardar(System.Collections.Hashtable campos, bool insertarNuevo, SessionService sesion)
        {
            try
            {
                ...
                {
                    precioCalculado =  PreciosService_new.RedondarPrecio((decimal)campos[ListaDePreciosModificarDetallesService.PrecioNuevo_NombreCol], decimal.Parse(dtCabecera.Rows[0][ListaDePreciosModificarService.ListaPreciosId_NombreCol].ToString()));
                ...
                if (campos.Contains(ListaDePreciosModificarDetallesService.PrecioNuevo_NombreCol) && campos.Contains(hashPorcentaje))
                {
                    ...
                    precioCalculado = PreciosService_new.RedondarPrecio(precioCalculado, decimal.Parse(dtCabecera.Rows[0][ListaDePreciosModificarService.ListaPreciosId_NombreCol].ToString()));
            ...
            while (existenHijos)
            {
                ...
                else
                {
                    ...
                    for (int a = 0; a < dtArticulos.Rows.Count; a++)
                    {
                        ...
                        decimal factorPorcentajeArticulo = (decimal)((decimal)dtArticulos.Rows[a][ArticulosService.PorcentajePrecioPadre_NombreCol]) / 100;
                      //  precio = precioPadre;
                        precio = precioPadre + (precioPadre * factorPorcentajeArticulo) + (decimal)dtArticulos.Rows[a][ArticulosService.MontoPrecioPadre_NombreCol];
                        if (precio < 0 )
                            throw new Exception("El precio del artículo '"+dtArticulos.Rows[a][ArticulosService.DescripcionInterna_NombreCol]+"' no puede ser menor a 0(cero).");
                        if (!insertarNuevo)
                        {
        ...
        // recibe como parametro id de articulo padreraiz: elimina el padre y todos sus hijos del detalle del caso.
        public void EliminarDetallesDePadreRaiz(decimal idListaPrecioModificar, string articulosId_list)
        {
            bool existenHijos = true;
            string filtroPadre = articulosId_list;
            decimal detalleId = Definiciones.Error.Valor.EnteroPositivo;
            while (existenHijos)
            {
                string auxList = ArticulosService.GetArticulosHijosPorArticulosId(filtroPadre);
                filtroPadre = auxList;
                ...
                {
                    articulosId_list += articulosId_list.Equals(string.Empty) ? auxList : ", " + auxList;
                }
            }
            string clausula = ListaDePreciosModificarDetallesService.ListaPreciosModificarId_NombreCol + " = " + idListaPrecioModificar + " and " + ListaDePreciosModificarDetallesService.ArticuloId_NombreCol + " in (" + articulosId_list + ")\n";
            ListaDePreciosModificarDetallesService lista = new ListaDePreciosModificarDetallesService();
            DataTable dtRegistrosEliminar = lista.GetListaDePreciosModificarDetallesDataTable(clausula, ListaDePreciosModificarDetallesService.Id_NombreCol);
            for (int a = 0; a < dtRegistrosEliminar.Rows.Count; a++)
            {
                detalleId = (decimal)dtRegistrosEliminar.Rows[a][ListaDePreciosModificarDetallesService.Id_NombreCol];
                this.Borrar(detalleId);
            }
        }

        #region GetArticulosSinListaDePrecio
        public static DataTable GetArticulosSinListaDePrecio(decimal lista_precio_modificar_id)
        {
            string clausulas = string.Empty;
            ListaDePreciosModificarDetallesService lista = new ListaDePreciosModificarDetallesService();

            DataTable dtDetalles = lista.GetListaDePreciosModificarDetallesDataTable(ListaDePreciosModificarDetallesService.ListaPreciosModificarId_NombreCol + " = " + lista_precio_modificar_id, string.Empty);
            if (dtDetalles.Rows.Count > 0)
                clausulas = " and " + ArticulosService.Id_NombreCol + " not in (" + dtDetalles.Rows[0][ListaDePreciosModificarDetallesService.ArticuloId_NombreCol];
            for (int i = 1; i < dtDetalles.Rows.Count; i++)
                clausulas += ", " + dtDetalles.Rows[i][ListaDePreciosModificarDetallesService.ArticuloId_NombreCol];
            ...
            using (SessionService sesion = new SessionService())
            {
                try
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append("select a." + ArticulosService.Id_NombreCol + " \n");
                    sql.Append("from " + ArticulosService.Nombre_Tabla + " a \n");
                    sql.Append("where a." + ArticulosService.ParaVenta_NombreCol + " = '" + Definiciones.SiNo.Si + "'               -- es para venta \n");
                    sql.Append("and a." + ArticulosService.ProduccionInterna_NombreCol + " = '" + Definiciones.SiNo.Si + "'          -- de produccion interna \n");
                    sql.Append("and a." + ArticulosService.Estado_NombreCol + " = '" + Definiciones.Estado.Activo + "'                     -- activos \n");
                    sql.Append("and not exists (select d." + ListasDePrecioDetalleService.ListaPrecioId_NombreCol + " from " + ListasDePrecioDetalleService.Nombre_Tabla + " d, "+ListasDePrecioService.Nombre_Tabla +" lp \n");
                    //sql.Append("                where d." + ListasDePrecioDetalleService.ListaPrecioId_NombreCol + " in ( " + VariablesSistemaEntidadService.GetValorString(Definiciones.VariablesDeSistema.ListaDePrecioBaseId) + ")  -- lista de precio base, definida en variables de sistema \n");
                    sql.Append("                where d." + ListasDePrecioDetalleService.ListaPrecioId_NombreCol + " = lp." + ListasDePrecioService.Id_NombreCol);
                    sql.Append("                and lp."+ ListasDePrecioService.Es_Base_NombreCol + " = '" + Definiciones.SiNo.Si+"'");
                    sql.Append("                and d." + ListasDePrecioDetalleService.ArticuloId_NombreCol + " = a." + ArticulosService.Id_NombreCol + ")");
                    sql.Append(clausulas);
                    return sesion.Db.EjecutarQuery(sql.ToString());
                }
                catch (Exception)
                { throw; }
            }
        }
        #endregion GetArticulosSinListaDePrecio

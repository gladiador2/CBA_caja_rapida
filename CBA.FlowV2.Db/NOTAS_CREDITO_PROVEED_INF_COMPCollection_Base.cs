// <fileinfo name="NOTAS_CREDITO_PROVEED_INF_COMPCollection_Base.cs">
//		<copyright>
//			All rights reserved.
//		</copyright>
//		<remarks>
...
{
	/// <summary>
	/// The base class for <see cref="NOTAS_CREDITO_PROVEED_INF_COMPCollection"/>. Provides methods 
	/// for common database view operations. 
	/// </summary>
	/// <remarks>
	/// Do not change this source code. Update the <see cref="NOTAS_CREDITO_PROVEED_INF_COMPCollection"/>
	/// class if you need to add or change some functionality.
	/// </remarks>
	public abstract class NOTAS_CREDITO_PROVEED_INF_COMPCollection_Base
	{
		// Constants
		...
		public const string FUNCIONARIO_RESPONS_NOMBREColumnName = "FUNCIONARIO_RESPONS_NOMBRE";
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="NOTAS_CREDITO_PROVEED_INF_COMPCollection_Base"/> 
		/// class with the specified <see cref="CBAV2"/>.
		/// </summary>
		/// <param name="db">The <see cref="CBAV2"/> object.</param>
		public NOTAS_CREDITO_PROVEED_INF_COMPCollection_Base(CBAV2 db)
		{
			_db = db;
		}
		...
		/// <summary>
		/// Gets an array of all records from the <c>NOTAS_CREDITO_PROVEED_INF_COMP</c> view.
		/// </summary>
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		public virtual NOTAS_CREDITO_PROVEED_INF_COMPRow[] GetAll()
		{
		...
		/// includes all records from the <c>NOTAS_CREDITO_PROVEED_INF_COMP</c> view.
		/// </summary>
		...
		/// to retrieve all records from the <c>NOTAS_CREDITO_PROVEED_INF_COMP</c> view.
		/// </summary>
		...
		/// <summary>
		/// Gets the first <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects that 
		/// match the search condition.
		/// </summary>
		/// <param name="whereSql">The SQL search condition. For example: 
		/// <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
		/// <returns>An instance of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> or null reference 
		/// (Nothing in Visual Basic) if the object was not found.</returns>
		public NOTAS_CREDITO_PROVEED_INF_COMPRow GetRow(string whereSql)
		{
			int totalRecordCount = -1;
			NOTAS_CREDITO_PROVEED_INF_COMPRow[] rows = GetAsArray(whereSql, null, 0, 1, ref totalRecordCount);
			return 0 == rows.Length ? null : rows[0];
		}

		/// <summary>
		/// Gets an array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects that 
		...
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		public NOTAS_CREDITO_PROVEED_INF_COMPRow[] GetAsArray(string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
		...
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		public virtual NOTAS_CREDITO_PROVEED_INF_COMPRow[] GetAsArray(string whereSql, string orderBySql,
		...
		protected virtual IDbCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			string sql = "SELECT * FROM TRC.NOTAS_CREDITO_PROVEED_INF_COMP";
		...
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		protected NOTAS_CREDITO_PROVEED_INF_COMPRow[] MapRecords(IDbCommand command)
		{
		...
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		protected NOTAS_CREDITO_PROVEED_INF_COMPRow[] MapRecords(IDataReader reader)
		{
			int totalRecordCount = -1;
		...
		/// of records in the reader object if 0 was passed into the method; otherwise it returns -1.</param>
		/// <returns>An array of <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> objects.</returns>
		protected virtual NOTAS_CREDITO_PROVEED_INF_COMPRow[] MapRecords(IDataReader reader, 
										int startIndex, int length, ref int totalRecordCount)
		{
			if(0 > startIndex)
			...
			int funcionario_respons_nombreColumnIndex = reader.GetOrdinal("FUNCIONARIO_RESPONS_NOMBRE");
			...
			while(reader.Read())
			{
				...
				if(ri > 0 && ri <= length)
				{
					NOTAS_CREDITO_PROVEED_INF_COMPRow record = new NOTAS_CREDITO_PROVEED_INF_COMPRow();
					...
					if(!reader.IsDBNull(funcionario_respons_nombreColumnIndex))
						record.FUNCIONARIO_RESPONS_NOMBRE = Convert.ToString(reader.GetValue(funcionario_respons_nombreColumnIndex));
			...
			return (NOTAS_CREDITO_PROVEED_INF_COMPRow[])(recordList.ToArray(typeof(NOTAS_CREDITO_PROVEED_INF_COMPRow)));
		}
		...
		/// <summary>
		/// Converts <see cref="System.Data.DataRow"/> to <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/>.
		/// </summary>
		/// <param name="row">The <see cref="System.Data.DataRow"/> object to be mapped.</param>
		/// <returns>A reference to the <see cref="NOTAS_CREDITO_PROVEED_INF_COMPRow"/> object.</returns>
		protected virtual NOTAS_CREDITO_PROVEED_INF_COMPRow MapRow(DataRow row)
		{
			NOTAS_CREDITO_PROVEED_INF_COMPRow mappedObject = new NOTAS_CREDITO_PROVEED_INF_COMPRow();
			...
			// Column "FUNCIONARIO_RESPONS_NOMBRE"
			dataColumn = dataTable.Columns["FUNCIONARIO_RESPONS_NOMBRE"];
			if(!row.IsNull(dataColumn))
				mappedObject.FUNCIONARIO_RESPONS_NOMBRE = (string)row[dataColumn];
		...
		/// the <c>NOTAS_CREDITO_PROVEED_INF_COMP</c> view.
		/// </summary>
		/// <returns>A reference to the <see cref="System.Data.DataTable"/> object.</returns>
		protected virtual DataTable CreateDataTable()
		{
			DataTable dataTable = new DataTable();
			dataTable.TableName = "NOTAS_CREDITO_PROVEED_INF_COMP";
			DataColumn dataColumn;
			...
			dataColumn.AllowDBNull = false;
			dataColumn.ReadOnly = true;
			dataColumn = dataTable.Columns.Add("FUNCIONARIO_RESPONS_NOMBRE", typeof(string));
		...
		protected virtual IDbDataParameter AddParameter(IDbCommand cmd, string paramName, object value)
		{
			...
			switch(paramName)
			{
				...
				case "FUNCIONARIO_RESPONS_NOMBRE":
					parameter = _db.AddParameter(cmd, paramName, DbType.AnsiString, value);
					break;
	...
	} // End of NOTAS_CREDITO_PROVEED_INF_COMPCollection_Base class
}  // End of namespace

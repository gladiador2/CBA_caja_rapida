// <fileinfo name="CAJA_SUCURSAL_REPARTOCollection_Base.cs">
//		<copyright>
//			All rights reserved.
//		</copyright>
//		<remarks>
...
{
	/// <summary>
	/// The base class for <see cref="CAJA_SUCURSAL_REPARTOCollection"/>. Provides methods 
	/// for common database view operations. 
	/// </summary>
	/// <remarks>
	/// Do not change this source code. Update the <see cref="CAJA_SUCURSAL_REPARTOCollection"/>
	/// class if you need to add or change some functionality.
	/// </remarks>
	public abstract class CAJA_SUCURSAL_REPARTOCollection_Base
	{
		// Constants
		public const string CAJA_IDColumnName = "CAJA_ID";
		public const string REPARTO_IDColumnName = "REPARTO_ID";
		public const string REPARTO_CASOColumnName = "REPARTO_CASO";
		public const string FECHA_INICIOColumnName = "FECHA_INICIO";
		public const string CHOFERColumnName = "CHOFER";
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="CAJA_SUCURSAL_REPARTOCollection_Base"/> 
		/// class with the specified <see cref="CBAV2"/>.
		/// </summary>
		/// <param name="db">The <see cref="CBAV2"/> object.</param>
		public CAJA_SUCURSAL_REPARTOCollection_Base(CBAV2 db)
		{
			_db = db;
		}
		...
		/// <summary>
		/// Gets an array of all records from the <c>CAJA_SUCURSAL_REPARTO</c> view.
		/// </summary>
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		public virtual CAJA_SUCURSAL_REPARTORow[] GetAll()
		{
		...
		/// includes all records from the <c>CAJA_SUCURSAL_REPARTO</c> view.
		/// </summary>
		...
		/// to retrieve all records from the <c>CAJA_SUCURSAL_REPARTO</c> view.
		/// </summary>
		...
		/// <summary>
		/// Gets the first <see cref="CAJA_SUCURSAL_REPARTORow"/> objects that 
		/// match the search condition.
		/// </summary>
		/// <param name="whereSql">The SQL search condition. For example: 
		/// <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
		/// <returns>An instance of <see cref="CAJA_SUCURSAL_REPARTORow"/> or null reference 
		/// (Nothing in Visual Basic) if the object was not found.</returns>
		public CAJA_SUCURSAL_REPARTORow GetRow(string whereSql)
		{
			int totalRecordCount = -1;
			CAJA_SUCURSAL_REPARTORow[] rows = GetAsArray(whereSql, null, 0, 1, ref totalRecordCount);
			return 0 == rows.Length ? null : rows[0];
		}

		/// <summary>
		/// Gets an array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects that 
		...
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		public CAJA_SUCURSAL_REPARTORow[] GetAsArray(string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
		...
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		public virtual CAJA_SUCURSAL_REPARTORow[] GetAsArray(string whereSql, string orderBySql,
		...
		protected virtual IDbCommand CreateGetCommand(string whereSql, string orderBySql)
		{
			...
			string sql = "SELECT * FROM TRC.CAJA_SUCURSAL_REPARTO";
		...
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		protected CAJA_SUCURSAL_REPARTORow[] MapRecords(IDbCommand command)
		{
		...
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		protected CAJA_SUCURSAL_REPARTORow[] MapRecords(IDataReader reader)
		{
			int totalRecordCount = -1;
		...
		/// of records in the reader object if 0 was passed into the method; otherwise it returns -1.</param>
		/// <returns>An array of <see cref="CAJA_SUCURSAL_REPARTORow"/> objects.</returns>
		protected virtual CAJA_SUCURSAL_REPARTORow[] MapRecords(IDataReader reader, 
										int startIndex, int length, ref int totalRecordCount)
		{
			if(0 > startIndex)
			...
			int caja_idColumnIndex = reader.GetOrdinal("CAJA_ID");
			int reparto_idColumnIndex = reader.GetOrdinal("REPARTO_ID");
			int reparto_casoColumnIndex = reader.GetOrdinal("REPARTO_CASO");
			int fecha_inicioColumnIndex = reader.GetOrdinal("FECHA_INICIO");
			int choferColumnIndex = reader.GetOrdinal("CHOFER");
			...
			while(reader.Read())
			{
				...
				if(ri > 0 && ri <= length)
				{
					CAJA_SUCURSAL_REPARTORow record = new CAJA_SUCURSAL_REPARTORow();
					recordList.Add(record);

					record.CAJA_ID = Math.Round(Convert.ToDecimal(reader.GetValue(caja_idColumnIndex)), 9);
					record.REPARTO_ID = Math.Round(Convert.ToDecimal(reader.GetValue(reparto_idColumnIndex)), 9);
					if(!reader.IsDBNull(reparto_casoColumnIndex))
						record.REPARTO_CASO = Math.Round(Convert.ToDecimal(reader.GetValue(reparto_casoColumnIndex)), 9);
					if(!reader.IsDBNull(fecha_inicioColumnIndex))
						record.FECHA_INICIO = Convert.ToString(reader.GetValue(fecha_inicioColumnIndex));
					if(!reader.IsDBNull(choferColumnIndex))
						record.CHOFER = Convert.ToString(reader.GetValue(choferColumnIndex));
			...
			return (CAJA_SUCURSAL_REPARTORow[])(recordList.ToArray(typeof(CAJA_SUCURSAL_REPARTORow)));
		}
		...
		/// <summary>
		/// Converts <see cref="System.Data.DataRow"/> to <see cref="CAJA_SUCURSAL_REPARTORow"/>.
		/// </summary>
		/// <param name="row">The <see cref="System.Data.DataRow"/> object to be mapped.</param>
		/// <returns>A reference to the <see cref="CAJA_SUCURSAL_REPARTORow"/> object.</returns>
		protected virtual CAJA_SUCURSAL_REPARTORow MapRow(DataRow row)
		{
			CAJA_SUCURSAL_REPARTORow mappedObject = new CAJA_SUCURSAL_REPARTORow();
			DataTable dataTable = row.Table;
			DataColumn dataColumn;
			// Column "CAJA_ID"
			dataColumn = dataTable.Columns["CAJA_ID"];
			if(!row.IsNull(dataColumn))
				mappedObject.CAJA_ID = (decimal)row[dataColumn];
			// Column "REPARTO_ID"
			dataColumn = dataTable.Columns["REPARTO_ID"];
			if(!row.IsNull(dataColumn))
				mappedObject.REPARTO_ID = (decimal)row[dataColumn];
			// Column "REPARTO_CASO"
			dataColumn = dataTable.Columns["REPARTO_CASO"];
			if(!row.IsNull(dataColumn))
				mappedObject.REPARTO_CASO = (decimal)row[dataColumn];
			// Column "FECHA_INICIO"
			dataColumn = dataTable.Columns["FECHA_INICIO"];
			if(!row.IsNull(dataColumn))
				mappedObject.FECHA_INICIO = (string)row[dataColumn];
			// Column "CHOFER"
			dataColumn = dataTable.Columns["CHOFER"];
			if(!row.IsNull(dataColumn))
				mappedObject.CHOFER = (string)row[dataColumn];
			// Column "VEHICULO"
		...
		/// the <c>CAJA_SUCURSAL_REPARTO</c> view.
		/// </summary>
		/// <returns>A reference to the <see cref="System.Data.DataTable"/> object.</returns>
		protected virtual DataTable CreateDataTable()
		{
			DataTable dataTable = new DataTable();
			dataTable.TableName = "CAJA_SUCURSAL_REPARTO";
			DataColumn dataColumn;
			dataColumn = dataTable.Columns.Add("CAJA_ID", typeof(decimal));
			dataColumn.AllowDBNull = false;
			dataColumn.ReadOnly = true;
			dataColumn = dataTable.Columns.Add("REPARTO_ID", typeof(decimal));
			dataColumn.AllowDBNull = false;
			dataColumn.ReadOnly = true;
			dataColumn = dataTable.Columns.Add("REPARTO_CASO", typeof(decimal));
			dataColumn.ReadOnly = true;
			dataColumn = dataTable.Columns.Add("FECHA_INICIO", typeof(string));
			dataColumn.MaxLength = 10;
			dataColumn.ReadOnly = true;
			dataColumn = dataTable.Columns.Add("CHOFER", typeof(string));
	...
	} // End of CAJA_SUCURSAL_REPARTOCollection_Base class
}  // End of namespace
